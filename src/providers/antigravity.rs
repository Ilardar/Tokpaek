...
use crate::diaglog::{dbg_log, diag}; use super::{Family, FetchError, Limit, LimitKind, LimitPool, LimitWindow, Snapshot, WeeklyQuota};
...
fn candidates() -> Vec<Endpoint> {
    ...
    files.sort_by_key(|a| std::cmp::Reverse(a.seen));
...
fn from_processes(out: &mut Vec<Endpoint>) {
    ...
    servers.sort_by_key(|a| std::cmp::Reverse(a.0)); // newest first
...
pub fn fetch() -> Result<Snapshot, FetchError> {
    ...
    for ep in &eps {
        ...
        if let Ok(summary) = call_quota_summary(ep) {
            if let Some(snap) = build_snapshot_from_summary(summary) {
                remember(ep);
                diag(&format!(
...
fn build_snapshot_from_summary(summary: QuotaSummaryData) -> Option<Snapshot> {
    let now = Utc::now();
    ...
    for group in summary.groups {
        ...
        if main_bucket.is_some() || weekly_exhausted {
            ...
            limits_by_group[group_idx] = Some(Limit {
                ...
                // A weekly lockout supersedes the session: the row then counts
                // against the weekly quota, and its kind says so.
                kind: if weekly_exhausted {
                    LimitKind::Weekly
                } else {
                    LimitKind::Session
                },
...
mod tests {
    ...
    fn parse_quota_summary_with_weekly_and_5h() {
        ...
        let snap = build_snapshot_from_summary(summary).unwrap();
...
mod weekly_tests {
    ...
    fn snapshot(buckets: serde_json::Value) -> Snapshot {
        ...
        .unwrap();
        build_snapshot_from_summary(summary).unwrap()
    }
